...
`);
  ...
  var socket = new Socket("/socket", { params: { token } });
  if (token != null) {
    socket.connect();
  }
  var channel_user = socket.channel("user", {});
  channel_user.join().receive("ok", (resp) => {
    console.log("Joined user successfully", resp);
  }).receive("error", (resp) => {
    console.log("Unable to join user", resp);
  });
  var user_socket_default = socket;

  // js/hooks.js
  var Hooks2 = {};
  Hooks2.Chat = {
    mounted() {
      let channel_forum = user_socket_default.channel("forum", {});
      channel_forum.join().receive("ok", (resp) => {
    ...
    destroyed() {
      user_socket_default.disconnect();
    },
    listenForMessages(channel) {
      ...
      function render_message(payload) {
        ...
        div.innerHTML = `
          <div class="flex mb-2 ${isCurrentUser ? "justify-end" : "justify-start"}">

                  <div class="flex flex-col max-w-96 rounded-lg p-3 gap-1 ${isCurrentUser ? "bg-purple-700 text-white" : "bg-white text-gray-700"}">
                
                  <div class="font-semibold">${isCurrentUser ? "" : payload.name}</div>
                    <p>${payload.message}</p>
                    <div class="text-xs ${isCurrentUser ? "text-gray-300 text-right" : "text-gray-500"}">
      ...
      function formatTime(datetime) {
        ...
        return ("0" + (m.getUTCMonth() + 1)).slice(-2) + "/" + ("0" + m.getUTCDate()).slice(-2) + " " + ("0" + m.getUTCHours()).slice(-2) + ":" + ("0" + m.getUTCMinutes()).slice(-2);
      }
    }
  };
  Hooks2.Scroll = {
  ...
  liveSocket.connect() >> liveSocket.enableDebug();
